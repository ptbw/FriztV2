...
int PCA9685_closeI2C(unsigned char addr)
{
	int retval = 0;
	if(fd > 0){
		retval = PCA9685_initPWM(fd, addr, 0);
		fd = -1;
	}
...
int set_PWM_PCA9685(uint8_t hwAddress, uint8_t pwmChannel, uint16_t onTime, uint16_t offTime){
	
    //fprintf(stderr, "addr %02x channel %02d ontime %02d offtime %02d\n",hwAddress, pwmChannel, onTime, offTime);
	...
	val = ((uint8_t)(onTime&0xFF));
	reg = LED0_ON_L+4*pwmChannel;	
    ret = _PCA9685_writeI2CReg(fd,hwAddress, reg, 1, &val);
	if (ret != 0) {
		fprintf(stderr, "PCA9685_setPWMVal(): _PCA9685_writeI2CReg() returned ");
		fprintf(stderr, "%d on addr %02x reg %02x val %02x\n", ret, hwAddress, reg, val);
		return -1;
	} // if 
  
	val = ((uint8_t)(onTime>>8));
	reg = LED0_ON_H+4*pwmChannel;
    ret = _PCA9685_writeI2CReg(fd,hwAddress, reg, 1, &val);
	if (ret != 0) {
		fprintf(stderr, "PCA9685_setPWMVal(): _PCA9685_writeI2CReg() returned ");
		fprintf(stderr, "%d on addr %02x reg %02x val %02x\n", ret, hwAddress, reg, val);
	...
	val = ((uint8_t)(offTime&0xFF));
	reg = LED0_OFF_L+4*pwmChannel;
    ret = _PCA9685_writeI2CReg(fd,hwAddress, reg, 1, &val);
	if (ret != 0) {
		fprintf(stderr, "PCA9685_setPWMVal(): _PCA9685_writeI2CReg() returned ");
		fprintf(stderr, "%d on addr %02x reg %02x val %02x\n", ret, hwAddress, reg, val);
	...
	val = ((uint8_t)(offTime>>8));	
	reg = LED0_OFF_H+4*pwmChannel;
    ret = _PCA9685_writeI2CReg(fd,hwAddress, reg, 1, &val);
	if (ret != 0) {
		fprintf(stderr, "PCA9685_setPWMVal(): _PCA9685_writeI2CReg() returned ");
		fprintf(stderr, "%d on addr %02x reg %02x val %02x\n", ret, hwAddress, reg, val);
